...
class LocalHeroLayer extends ContainerLayer {
  ...
  ///
  /// If the [] was not linked to a [LeaderLayer], or if this layer has
  ...
  void _establishTransform() {
    ...
    assert(
      ...
      "Linked LeaderLayer anchor is not in the same "
      "layer tree as the FollowerLayer.",
    );
  ...
  ///
  /// A [FollowerLayer] copies changes from a [LeaderLayer]
  /// that could be anywhere
  /// in the Layer tree, and that leader layer could
  /// change without notifying the
  /// follower layer. Therefore we have to always call a follower layer's
  /// [addToScene]. In order to call follower
  /// layer's [addToScene], leader layer's
  ...
  void applyTransform(Layer? child, Matrix4 transform) {
    assert(child != null, "Exception child");
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties
      ..add(DiagnosticsProperty<LocalHeroController>("controller", controller))
      ..add(
        TransformProperty(
          "transform",
          getLastTransform(),
